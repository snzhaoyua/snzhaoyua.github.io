...
1. 不能 创建对象，数组。
1. 不能 抛出、捕获异常。
1. 不能 调用任意实例或静态方法，只能调用 BTraceUtils 中的方法。
1. 不能 修改目标程序的静态或实例变量，不过 BTrace 程序自己不做限制。
1. 不能 有实例变量或方法，方法不能有返回值类型，BTrace 程序的所有方法必须是 public static 1. oid 的，所有的字段都必须是 static 的。
1. 不能 有 outer, inner, nested 或 local 类。
1. 不能 用 synchronized 关键字。
1. 不能 有循环 (for, while, do..while)。
1. 不能 继承任何类 (即父类只能是 java.lang.Object)。
1. 不能 实现接口。
1. 不能 包含 assert 语句.
1. 不能 使用 class literals.
