...
解决办法有三种：
1. 继续使用ssl而不需要配置证书：jdbc去掉useSSL=false即可(sum在application.properties里)，但是参考典型1，运行日志会一直报警告。
2. 继续使用ssl并且配置证书：参考`以notice为例，如何解决 jdbc 客户端的警告信息，以及开启单、双向认证`
2. 不使用ssl：参考典型1，在ssl无法创建的时候，需要在jdbc串增加此配置 allowPublicKeyRetrieval=true&useSSL=false
